...
> 翻訳は [`PRIVACY.vi.md`](PRIVACY.vi.md)、[`PRIVACY.zh.md`](PRIVACY.zh.md)、
> [`PRIVACY.ja.md`](PRIVACY.ja.md) にある。英語版が正文。
...
**Deskhub は個人データを収集、保存、販売、共有しない。当方はサーバを一切運用しておらず、
利用者または利用状況に関するデータが本ソフトウェアを通じて当方または第三者に届くことは
ない。** 本ソフトウェアにはユーザーアカウント、analytics、crash reporting、広告はなく、
第三者の SDK も組み込まれていない。
...
本ソフトウェアは次のものにアクセスせず、処理もしない。利用者の氏名（上記のデバイス名を
除く。その既定値はコンピュータまたは端末自身の名称である）、メールアドレス、電話番号、
連絡先、位置情報、写真、ファイル（stream すると選択した PC の画面に表示されているものを
除く）、microphone、カメラ、広告 ID、および OS が app を動作させるために必要な範囲を
超える端末 ID。
...
該当なし。利用者のデータが本ソフトウェアを通じて利用者自身の端末と network の外へ出る
ことはない。

...
| H-18 | Passcode in plain sight | The hosting screen shows the pairing passcode on its own — spaced digits, large and monospaced — with a **Copy passcode** button that puts the bare code on the clipboard. The code is no longer quoted inside the sharing status; that line keeps the explanation only. With no passcode set the panel reads *None* and offers no copy button. The passcode itself is edited on the **Settings** page beside the other security settings, on every platform. |

...
- **保留规则才是全部答案，而只在一个工作点上读它掩盖了这一点**：上一段断定 NACK-only 是死路。
  ...
  两个方向都动，有的点变大有的点变小，因为少要几次关键帧足以抵掉更长的等待。**在产默认值没有
  改变**：只有调用方抬高上限时才够得着这段推导，于是扫描能跑它，而生产的行为仍与测量时一致。
  在单一工作点上得到的否定结论，只是关于那个点的陈述，而这一次它掩盖了三倍的差距。
...
- **接收端合并是发送端的镜像，代价是读取契约里多一个字段**：P2 剩下的一半是 Linux 上的 GRO 和
  ...
  样的形状用 `WSARecvMsg` 和 `UDP_COALESCED_INFO` 写好了，但**没有经过任何编译器，也没有碰过任何
  网卡**——上面那张 loopback 表是 Linux 的数字，不能挪用。

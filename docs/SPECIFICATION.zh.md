...
| H-12 | 表中的 shell session | terminal 共享期间，实时表中显示一行 *Terminal* 及其 port，每个已打开的 shell 在其下各占一行，标识方式与 viewer 相同（C-7），并带 *Disconnect*。*Terminal* 行上的 *Stop* 仅结束 terminal 共享。同时最多打开 **8** 个 shell。shell 的打开、关闭、reattach 与过期均连同 client 的地址、名称和 key 写入 session log（G-3）。 |
| H-13 | shell 可在连接中断后保留 | 连接中断的 shell 会被保留 **2 分钟**，以便同一台机器在 session 内容完整的情况下 reattach；超时后予以丢弃。client 自行执行 reattach：在 host 仍可能保留该 shell 的时间范围内，以逐步延长的间隔重试，重试期间明确显示正在 reattach，并连同内容与 scrollback 接回同一个 shell。仅当 2 分钟结束后，client 才报告连接丢失，并提示重新开始。 |
...
| H-14 | Stop & attach（桌面） | 每一行 shell，无论处于活动状态还是等待 reattach，都另有 **Stop & attach**：远端 client 被断开（其窗口报告 shell 已结束），同一个 shell 在 host 的 terminal 窗口中打开，内容与 scrollback 保持不变。此后该 shell 归属于 host：原 client 无法再 reattach，2 分钟限制（H-13）不再适用，表中该行标记为 *attached on this machine*，关闭 host 上的窗口或点击该行的 *Stop* 即结束该 shell。此次接管同样写入 session log（G-3）。 |
...
| V-8 | 自动重连 | 失去 host 的 session —— network 中断，stream 停止 —— 不会立即结束。窗口保留最后一帧画面，显示正在 reattach，并以 backoff 方式重连最多一分钟；host 再次应答后画面自动恢复。仅在该时间之后，或 host 主动结束、拒绝该 session 时，窗口才会连同原因关闭。shell 窗口保留其自身两分钟的 reattach 宽限期。 |

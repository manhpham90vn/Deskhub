...
| H-18 | Passcode hiện rõ ngay trên màn hình | Màn hình host hiển thị passcode ghép đôi riêng một chỗ — các chữ số cách nhau, cỡ lớn, font monospace — kèm nút **Copy passcode** đặt mã thuần vào clipboard. Mã không còn được ghép vào câu trạng thái share nữa; câu đó chỉ còn phần giải thích. Khi không đặt passcode, khung ghi *None* và không có nút copy. Việc sửa passcode nằm ở trang **Settings**, cạnh các thiết lập bảo mật khác, trên mọi nền tảng. |

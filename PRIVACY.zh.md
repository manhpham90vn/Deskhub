...
> 译本见 [`PRIVACY.vi.md`](PRIVACY.vi.md)、[`PRIVACY.zh.md`](PRIVACY.zh.md) 和
> [`PRIVACY.ja.md`](PRIVACY.ja.md)。以英文版为准。
...
**Deskhub 不收集、不存储、不出售、不分享任何个人数据。我们不运营任何服务器，关于你或你
使用情况的任何数据都不会通过本软件到达我们或任何第三方。** 本软件没有用户账号、
analytics、crash reporting 与广告，也未嵌入任何第三方 SDK。
...
本软件不访问也不处理：你的姓名（上述设备名除外，其默认值取自电脑或设备自身的名称）、
电子邮件地址、电话号码、通讯录、位置、照片、文件（你选择 stream 的 PC 屏幕上可见的内容
除外）、microphone、摄像头、广告标识符，以及任何超出操作系统运行本 app 所需范围的设备
标识符。
...
无。你的数据不会通过本软件离开你自己的设备与 network。
